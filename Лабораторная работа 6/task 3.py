...
def to_csv_file(filename: str, headers: list, rows: list, delimiter=',',
                new_line='\n') -> None:
    with open(filename, 'wt') as f:
        f.write(delimiter.join(headers) + new_line)  # Добавляем шапку таблицы

        for line_ in rows:
            f.writelines(delimiter.join(line_) + new_line) # Добавляем элементы
